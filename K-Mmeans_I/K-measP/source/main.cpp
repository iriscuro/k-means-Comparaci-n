...
public:
    ...
    void combinar(const KMeans<T>& other);
    void Combinar_(const std::vector<KMeans<T>>& kmeansList);
...
void KMeans<T>::ejecutar(const std::vector<Punto<T>>& puntos) {
    ...
    while (cambios) {
        ...
        // Asignar cada punto al cluster más cercano en paralelo
        #pragma omp parallel for
        for (int i = 0; i < puntos.size(); ++i) {
            int clusterActual = 0;
            T distanciaMinima = calcularDistancia(puntos[i], clusters[0].centroide);

            for (int j = 1; j < clusters.size(); ++j) {
                T distancia = calcularDistancia(puntos[i], clusters[j].centroide);
            ...
            #pragma omp critical
            {
                clusters[clusterActual].agregarPunto(puntos[i]); // Asignar el punto al cluster más cercano
            }
        }

        // Actualizar los centroides de los clusters en paralelo
        #pragma omp parallel for
        for (int i = 0; i < clusters.size(); ++i) {
            if (!clusters[i].puntos.empty()) {
                T sumX = 0, sumY = 0;
                for (const auto& punto : clusters[i].puntos) {
                    sumX += punto.x;
                    sumY += punto.y;
                }
                T nuevoCentroideX = sumX / clusters[i].puntos.size();
                T nuevoCentroideY = sumY / clusters[i].puntos.size();

                #pragma omp critical
                {
                    clusters[i].centroide = Punto<T>(nuevoCentroideX, nuevoCentroideY);
                }
            }
        }
    }
...
void KMeans<T>::dibujarClusters() const {
    ...
    output << "</head>\n";
    output << "<body>\n";
    output << "<div id=\"map\" style=\"width: 1500px; height: 700px;\"></div>\n";
    output << "<script>\n";
    output << "var puntos = [\n";
...
// Método para combinar los resultados de diferentes instancias de KMeans
template<typename T>
void KMeans<T>::Combinar_(const std::vector<KMeans<T>>& kmeansList) {
   return ;
}
...
void dibujarPresentacion() {
    ...
    std::cout << "====================================" << std::endl;
    std::cout << "K-means Parallel Version" << std::endl;
    std::cout << "====================================" << std::endl;
}
